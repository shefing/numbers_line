  ...
  return (
    <div className="relative flex flex-col items-center">
      ...
      <div className="m-[18px] flex flex-col items-end shadow-2xl rounded-[6px] border border-solid border-[#009FDE] p-2">
        <Button className={MatchDisplayButtonClassName(TypeCover.allCover, TypeCover.allCover)} onClick={() => onClickButtons(TypeCover.allCover)}>
          הסתר הכל
        </Button>
        <Button className={buttonEyeClassName} onClick={() => onClickButtons(TypeCover.randomly)}>
          הסתר חלקית
        </Button>
        <Button className={MatchDisplayButtonClassName(TypeCover.allCover, TypeCover.partiallyCover)} onClick={() => onClickButtons(TypeCover.partiallyCover)}>
          הסתר ידנית
        </Button>
        <Button
          className={MatchDisplayButtonClassName(TypeCover.allDiscover, TypeCover.partiallyDiscover)}
          onClick={() => onClickButtons(TypeCover.partiallyDiscover)}
        >
          הצג ידנית
        </Button>
        <Button className={MatchDisplayButtonClassName(TypeCover.allDiscover, TypeCover.allDiscover)} onClick={() => onClickButtons(TypeCover.allDiscover)}>
          הצג הכל
        </Button>
      </div>
    </div>
